...
func createInterface(fd uintptr, ifName string, flags uint16) (createdIFName string, err error) {
	...
	fd2, err := syscall.Socket(syscall.AF_INET, syscall.SOCK_DGRAM, syscall.IPPROTO_IP)
	if err != nil {
		return
	}
	err = ioctl(uintptr(fd2), syscall.SIOCGIFFLAGS, uintptr(unsafe.Pointer(&req)))
	if err != nil {
		err = nil
		return
	}
	req.Flags |= syscall.IFF_UP | syscall.IFF_RUNNING
	err = ioctl(uintptr(fd2), syscall.SIOCSIFFLAGS, uintptr(unsafe.Pointer(&req)))
	if err != nil {
		err = nil
		return
	}
